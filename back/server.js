...
const port = 5000;
// CORSミドルウェアを適用
app.use(cors({origin:["https://www.global-webcreation.com", ]}));
// これCORSエラー
...
const append = async (name, email, message) => {
    ...
    try {
        await sheets.spreadsheets.values.append({
            ...
            range: `就活テストAcademy!A:H`, //書き込む範囲を指定
            valueInputOption: 'RAW',
            requestBody: { 
                values: [ 
                    [name, email, message], // 書き込むデータを指定
                ]
            }
        });
    } catch (error) {
...
// 受け取り方 POST data:{name:string, email:string, message: string}
app.post('/', (req, res) => {
